...
temporary <- temporary %>%
  mutate(gscore = (percentwhite.zscore + percentbach.zscore + percentrent.zscore)/3)
...
temporary2 <- temporary2 %>%
  mutate(gscore2= (percentwhite.zscore + percentbach.zscore + percentrent.zscore)/3)
...
###Gentrification in King County, Washington 2009
```{r echo=FALSE,  message=FALSE, warning=FALSE}
#get range for data
...
###Change in Gentrification in King County, Washington 2009 - 2017
...
m3 <- m3 %>% 
    ...
    title = "Change Gentrification Index",
    opacity = .7
  ) 
m3
